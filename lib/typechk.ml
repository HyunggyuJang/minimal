(* $Id: typechk.ml,v 1.15 2018/06/26 01:30:27 garrigue Exp $ *)
      ...
      raise
        (Type_error
           ( Multiple_occurence
               ...
               , if defkind = "" then "definition" else defkind ^ " definition"
               )
           , loc ))
...
let rec type_expr def sty =
  ...
  | STconstr (s, stl) ->
    (try
       ...
       if List.length stl <> List.length info.ti_params
       then error_message sty.st_loc ("wrong arity for type " ^ s);
       subst
...
let type_kind names vars kind =
  ...
  match kind with
  | SKabbrev sty -> Kabbrev (type_expr true sty)
  | SKvariant cl ->
    new_constrs := List.map fst cl @ !new_constrs;
    Kvariant (List.map (fun (s, stl) -> s, List.map (type_expr true) stl) cl)
  | SKrecord fl ->
    ...
    List.iter
      (fun td ->
        ...
        try
          add_type
            td.sd_name
            { ti_params = info.ti_params; ti_res = info.ti_res; ti_kind = kind }
        with
        | exn ->
...
let rec pattern pat =
  ...
  | SPid s ->
    (try
       ...
       if info.ci_args <> []
       then
         error_message pat.sp_loc ("constructor " ^ s ^ " requires an argument");
  ...
  | SPconstr (s, spat) ->
    (try
       ...
       match ty_args with
       | [] ->
         error_message pat.sp_loc ("constructor " ^ s ^ " takes no argument")
       | [ ty_arg ] ->
      ...
      List.split
        (List.map
           (fun lab ->
             ...
             with
             | Not_found -> error_message pat.sp_loc ("undefined label " ^ lab))
           labl)
...
let rec is_nonexpansive e =
  ...
  | SErecord l ->
    ...
    List.for_all
      (fun lab -> (Hashtbl.find labels lab).li_access <> Mutable)
      labl
...
let rec expression bnds e =
  ...
  | SEid s when is_constructor e ->
    ...
    if info.ci_args <> []
    then
      error_message e.se_loc ("wrong number of arguments for constructor " ^ s);
    let _, ty_res = instanciate_constr info in
    ty_res
  | SEid s ->
    (try
       let info = StrMap.find s bnds in
       if info.vi_access = Forbidden
        ...
        (List.map
           (fun lab ->
             try
               ...
               instanciate_label info
             with
             | Not_found -> error_message e.se_loc ("undefined label " ^ lab))
           labl)
    and ty_res = newvar () in
    if List.length labl <> (Hashtbl.find labels (List.hd labl)).li_total
    then error_message e.se_loc "some labels are missing";
    List.iter (unify_exp e ty_res) ty_rec;
    List.iter2 (type_expect bnds) ty_fields el;
    ty_res
  | SEapply (({ se_desc = SEid s; _ } as e1), el) when is_constructor e1 ->
    let info = Hashtbl.find constructors s in
    let ty_args, ty_res = instanciate_constr info in
  ...
  | SEapply
    ...
    let ty_res =
      if List.exists (fun ty -> repr ty = float) ty_args
      then (
        e1.se_desc <- SEid (op ^ ".");
        float)
      else int
    ...
    if op = "~" || List.length el = 2 then ty_res else Tarrow (ty_res, ty_res)
  | SEapply (e1, el) ->
    let ty1 = expression bnds e1 in
    let _ =
      try filter_arrow ty1 with
      | Unify -> error_message e1.se_loc "this expression is not a function"
    in
    let ty_args, ty_res =
      try
        List.fold_left
          ...
          el
      with
      | Unify -> error_message e1.se_loc "too many arguments for this function"
    in
    List.iter2 (type_expect bnds) (List.rev ty_args) el;
    ty_res
  | SEfunct (patl, e1) ->
    let _, { sp_loc = { last; _ }; _ } = split_last patl in
    let ty_args, new_bnds =
      pattern_list { first = (List.hd patl).sp_loc.first; last } patl
    in
    let bnds =
      ref (List.fold_left (fun b (k, d) -> StrMap.add k d b) bnds new_bnds)
    in
    StrMap.iter
      (fun s info ->
        if info.vi_access = Mutable
        then
          bnds
            := StrMap.add
                 s
  ...
  | SEset (s, e1) ->
    (try
       let info = StrMap.find s bnds in
       if info.vi_access = Immutable
       then error_message e.se_loc (s ^ " is not a mutable variable");
       if info.vi_access = Forbidden
       then error_message e.se_loc (s ^ " cannot be accessed here");
  ...
  | SEsetfield (e1, s, e2) ->
    (try
       let info = Hashtbl.find labels s in
       if info.li_access <> Mutable
       then error_message e.se_loc ("field " ^ s ^ " is not mutable");
  ...
  | SEfor (s, e1, _, e2, e3) ->
    ...
    let bnds =
      StrMap.add s { vi_type = type_int; vi_access = Immutable } bnds
    in
    ignore (statement bnds e3);
    Ttuple []
  | SEwhile (e1, e2) ->
    type_expect bnds (List.assoc "bool" basic_types).ti_res e1;
    ignore (statement bnds e2);
    Ttuple []
  | SEtype (e1, sty) ->
...
and type_expect bnds ty_expected e =
  ...
  | Ttuple tyl, SEtuple el -> List.iter2 (type_expect bnds) tyl el
  | _ ->
...
and command bnds cmd =
  let new_bnds =
    ...
    | SEfun l ->
      ...
      let tmp_bnds =
        List.map
          (fun s -> s, { vi_type = newvar (); vi_access = Immutable })
          names
      in
      let bnds =
        List.fold_left (fun b (k, d) -> StrMap.add k d b) bnds tmp_bnds
      in
      let new_bnds =
        List.map2
          (fun (s, info) e ->
...
let report_error = function
  ...
  | Type_mismatch (kind, ty1, ty2) ->
    Printer.reset_vars ();
    printf
      "@ @[<hov 2>%s has type@ %a@ where@ %a was expected@]"
  ...
  | Multiple_occurence (defkind, s) ->
    printf
      "@ @[<hov 2>%s occurs multiply in@ %s@]"
      (String.capitalize_ascii defkind)
      s
;;
