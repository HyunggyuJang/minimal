(* $Id: builtins.ml,v 1.25 2018/06/26 01:30:27 garrigue Exp $ *)
...
let handlers = ref []
let handle exn = List.exists (fun handler -> handler exn) !handlers
  ...
  := [ (fun exn ->
         let msg =
           ...
           | Compile.Match_error s ->
             if s <> ""
             then "Match failure in " ^ s
             else "Match failure in toplevel input"
           | Not_found -> "Internal : Not_found"
           | Invalid_argument s when s = "vect_item" ->
             "Error : array access out of bounds"
           | Invalid_argument s when s = "vect_assign" ->
             "Error : array assignment out of bounds"
           | Invalid_argument s -> "Invalid argument for " ^ s
...
and asin x =
  if x < -1. || x > 1. then raise (Invalid_argument "asin") else asin x

and acos x =
  if x < -1. || x > 1. then raise (Invalid_argument "acos") else acos x

and div_float x y = if y = 0. then raise Division_by_zero else x /. y
...
let trace n s =
  ...
  try
    ...
    let old_obj =
      try IdMap.find id !trace_table with
      | Not_found -> !obj_ref
    in
    let is_function =
      match Unify.repr info.vi_type with
      | Tarrow _ -> true
      | _ -> false
    in
    if not is_function
    then Format.printf "%s is not a function, you cannot trace it.@." s
    else (
      let traced_obj =
        if n <= 1
        then trace_obj s info.vi_type old_obj
  ...
  with
  | Not_found -> Format.printf "Unknown identifier %s.@." s
;;
...
let untrace s =
  ...
  with
  | Not_found -> Format.printf "Function %s is not traced.@." s
;;
...
let write_file name data =
  ...
  List.iter
    (fun x ->
      output_string oc (string_of_array x);
      output_char oc '\n')
    data;
  close_out oc
;;
...
let builtins =
  ref
    ...
    ; "=", repr (fun x y -> compare x y = 0), arr2 a a bool
    ; "<>", repr (fun x y -> compare x y <> 0), arr2 a a bool
    ; "==", repr ( == ), arr2 a a bool
    ; "!=", repr ( != ), arr2 a a bool
    ; "<", repr (fun x y -> compare x y < 0), arr2 a a bool
    ...
    ; "or", repr (fun a b -> a || b), arr2 bool bool bool
    ...
    ; (* "get_field", repr vect_item ; "set_field", repr vect_assign ; *)
      "length", repr Array.length, arr (array a) int
    ; "array", repr Array.make, arr2 int a (array a)
    ...
    ; ( "read_string"
      , repr (fun () -> array_of_string (read_line ()))
      , arr unit string )
    ; ( "string_of_int"
      , repr (fun n -> array_of_string (string_of_int n))
      , arr int string )
    ; ( "string_of_float"
      , repr (fun x -> array_of_string (string_of_float x))
      , arr float string )
    ; "format_float", repr format_float, arr2 int bool (arr float string)
    ; ( "int_of_string"
      , repr (fun n -> int_of_string (string_of_array n))
      , arr string int )
    ; ( "float_of_string"
      , repr (fun x -> float_of_string (string_of_array x))
      , arr string float )
...
let add_builtins () =
  List.iter
    (fun (name, obj, ty) ->
      add_value (new_id name) obj { vi_type = ty; vi_access = Immutable })
    !builtins
;;
