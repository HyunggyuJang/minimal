(* $Id: printer.ml,v 1.7 2018/06/26 01:30:27 garrigue Exp $ *)
...
let name_of_var tv =
  ...
  | Not_found ->
    let name =
      let n = !var_counter / 26
      and c = !var_counter mod 26 in
      if n > 0
      then String.make 1 (char_of_int (c + 97)) ^ string_of_int n
      else String.make 1 (char_of_int (c + 97))
    in
    incr var_counter;
    vars := (tv, name) :: !vars;
    name
...
let rec type_expr prio sch ty =
  ...
  | Ttuple l ->
    ...
    if prio > 1 then print_string ")";
    close_box ()
  | Tconstr (id, []) -> print_string id.name
  | Tconstr (id1, [ ty ])
...
and print_type ty = type_expr 0 false ty
...
let rec print_val prio depth ty obj =
  ...
  | Ttuple l ->
    ...
    print_string ")";
    close_box ()
  | Tconstr (id, []) when same_id id id_int -> print_int (Obj.obj obj)
  ...
  | Tconstr (id, [ ty ]) when same_id id id_list ->
    open_hovbox 1;
    print_string "[";
    cautious (print_list (print_val 0 (depth - 1) ty) print_comma) (Obj.obj obj);
    print_string "]";
    close_box ()
  | Tconstr (id, tyl) ->
    (try
...
let print_value ty obj =
  ...
  try print_val 0 !max_printer_depth ty obj with
  | x ->
    print_newline ();
    flush stderr;
    raise x
