(* $Id: compile.ml,v 1.17 2021/12/17 10:09:36 garrigue Exp $ *)
...
let empty_alloc () =
  { size = 0; table = IdMap.empty; defined = IdSet.empty; needed = IdSet.empty }
;;

let new_alloc alloc =
  { size = 0
  ; table = IdMap.empty
  ; defined = alloc.defined
  ; needed = IdSet.empty
  }
;;
...
let find_alloc alloc id =
  if IdSet.mem id alloc.defined
  then (
    try IdMap.find id alloc.table with
    | Not_found ->
...
let rec pattern alloc = function
  | UPid id ->
    ...
    CPid n
  | UPconst (Cfloat x) -> CPconst (Obj.repr x)
  | UPconst c -> CPint (Obj.obj (repr_constant c))
  | UPblock (tag, []) -> CPint (Obj.obj (Obj.new_block tag 0))
  | UPblock (tag, l) ->
...
let matching alloc exn pat =
  ...
  let rec match_rec = function
    | CPany -> fun _ _ -> ()
    | CPid n -> fun env obj -> env.(n) <- obj
    | CPint x -> fun _ obj -> if x <> Obj.obj obj then raise exn
    | CPconst x -> fun _ obj -> if x <> obj then raise exn
    | CPblock (tag, arr) ->
      fun env obj ->
        let len = Array.length arr in
        if Obj.is_int obj || Obj.size obj <> len || Obj.tag obj <> tag
        then raise exn;
        for i = 0 to len - 1 do
...
let rec command alloc = function
  ...
  | UEfun l ->
    ...
    (try
       let orefs = Array.map (fun (id, _) -> IdMap.find id !global_env) v in
...
and expression alloc = function
  ...
  | UEblock (tag, l) ->
    ...
    fun env ->
      ...
      obj
  | UEapply (UEid { name = ("&" | "or") as op; _ }, [ e1; e2 ]) ->
    let code1 = expression alloc e1
    and code2 = expression alloc e2 in
    if op = "&"
    then fun env -> Obj.repr (Obj.obj (code1 env) && Obj.obj (code2 env))
    else fun env -> Obj.repr (Obj.obj (code1 env) || Obj.obj (code2 env))
  | UEapply (e, [ e1 ]) ->
    let code = expression alloc e
    and code1 = expression alloc e1 in
    fun env -> (Obj.obj (code env) : Obj.t -> Obj.t) (code1 env)
  | UEapply (e, [ e1; e2 ]) ->
    let code = expression alloc e
    and code1 = expression alloc e1
    and code2 = expression alloc e2 in
    fun env ->
      (Obj.obj (code env) : Obj.t -> Obj.t -> Obj.t) (code1 env) (code2 env)
  | UEapply (e, l) ->
    let codes = List.map (expression alloc) l
    and code = expression alloc e in
    let rec apply env obj = function
      | [] -> obj
      | code :: l -> apply env ((Obj.obj obj : Obj.t -> Obj.t) (code env)) l
    in
  ...
  | UEfunct (ids, e) ->
    ...
    then
      ...
      let rec make_code n code =
        if n = 0
        then (Obj.magic code : Obj.t array -> Obj.t)
        else
          make_code
            (pred n)
  ...
  | UEcase (e, cases, err) ->
    ...
    List.iter
      (fun (pat, e) ->
        let _i = matching alloc' Match_failure pat
        and _j = expression alloc' e in
        ())
      cases;
    let new_needed = IdSet.diff alloc'.needed alloc.needed in
    IdSet.iter
      (fun id ->
        let _ = find_alloc alloc id in
        ())
      new_needed;
    ...
    then (fun env ->
      env.(i) <- code env;
      try_matching env codes)
    else fun env -> try_matching env codes
  | UEifthenelse (e, e1, e2) ->
    let code = expression alloc e
    and code1 = expression alloc e1
    and code2 = expression alloc e2 in
    fun env -> if Obj.obj (code env) then code1 env else code2 env
  | UEset (id, e) ->
    let code = expression alloc e in
    (try
       let i = find_alloc alloc id in
  ...
  | UEfor (id, e1, dir, e2, e) ->
    ...
    then
      fun env ->
      Obj.repr
        (for j = Obj.obj (code1 env) to Obj.obj (code2 env) do
           env.(i) <- Obj.repr j;
           (* build a cell to force allocation, hence signals *)
      ...
      fun env ->
      Obj.repr
        (for j = Obj.obj (code1 env) downto Obj.obj (code2 env) do
           env.(i) <- Obj.repr j;
           ignore (Some (code env))
         done)
  | UEwhile (e1, e2) ->
    ...
    fun env ->
      Obj.repr
        (while Obj.obj (code1 env) do
           ignore (Some (code2 env))
         done)
;;
