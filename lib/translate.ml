(* $Id: translate.ml,v 1.6 2018/06/26 01:30:27 garrigue Exp $ *)
...
let rec pattern pat =
  ...
  | SPconstr (s, pat) ->
    (try
       let info = Hashtbl.find constructors s in
       if info.ci_size = 0
       then failwith ("In translate__pattern, " ^ s ^ " : cannot be applied")
       else if info.ci_size = 1
       then (
...
let rec command bnds cmd =
  ...
  | SEvar l ->
    ...
    let bnds = List.combine sl ids @ bnds in
    [ UEfun (List.combine ids uel) ], bnds
  | STtype _ | SThide _ -> failwith "translate__command"
...
and expression bnds se =
  ...
  | SErecord l ->
    let arr = Array.make (List.length l) (UEconst (Cint 0)) in
    List.iter
      (fun (s, e1) ->
  ...
  | SEapply (({ se_desc = SEid s; _ } as e0), [ e ]) ->
    (try
       let info = Hashtbl.find constructors s in
       if info.ci_size = 0
       then failwith ("In translate__expression, " ^ s ^ " : cannot be applied")
       else if info.ci_size = 1
       then UEblock (info.ci_tag, [ expression bnds e ])
       else (
  ...
  | SEsetfield (e1, s, e2) ->
    ...
    and info = Hashtbl.find labels s in
    UEapply
      (UEid (List.assoc "$.<-" bnds), [ ue1; UEconst (Cint info.li_index); ue2 ])
  | SEfor (s, e1, dir, e2, e3) ->
    let ue1 = expression bnds e1
    and ue2 = expression bnds e2 in
    let id = new_id s in
    UEfor (id, ue1, dir, ue2, expression ((s, id) :: bnds) e3)
  | SEwhile (e1, e2) -> UEwhile (expression bnds e1, expression bnds e2)
  | SEtype (e, _) -> expression bnds e
;;
