...
let output_location filename lines oc loc =
  ...
  if line = line' then 
    let pos = min (String.length s) (loc.first - offset) in
    let len = min (String.length s - pos) (loc.last - loc.first) in
    ...
    and s2 =
      ...
      let pos' =
        ...
        try skip_forward s' (String.index_from s' pos0 ' ') pos2
        with Not_found -> skip_forward s' pos0 pos2
      in
